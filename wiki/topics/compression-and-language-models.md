...
- **LLMs as general-purpose compressors.** Excluding model parameter bytes, Chinchilla 70B, trained on text, compresses ImageNet patches to 43.4% and LibriSpeech to 16.4%, beating the compared domain-specific codecs on raw compression rate. This is achieved through in-context learning, not cross-modal training ([paper](../sources/language-modeling-is-compression.md)).
