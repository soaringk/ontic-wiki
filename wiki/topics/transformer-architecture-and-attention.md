...
- Decode is slower and more sequential because each new token depends on the model's available causal context or recurrent state.
...
- FlashAttention-2 and FlashAttention-3 show that attention-kernel performance depends on more than asymptotic complexity: sequence-block parallelism, warp work partitioning, asynchronous data movement, GEMM-softmax overlap, and low-precision layout constraints all affect usable long-context throughput. Standard-precision paths preserve exact dense-attention semantics, while FlashAttention-3's FP8 path is approximate.
