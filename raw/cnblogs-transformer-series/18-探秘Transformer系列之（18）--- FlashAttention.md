...
source: "https://www.cnblogs.com/rossiXYZ/p/18791822"
site: "博客园"
domain: "cnblogs.com"
author: "罗西的思考"
...
更新迭代到此时的全局求和结果$ l\_{all}^{new} = e^{m\_{max} - m\_{max}^{new}}\*l\_{all} + e <sup>{m(x</sup>) - m\_{max} <sup>{new}}*l(x</sup>)\\(。\\)l\\(并不能直接累加更新，因为不同的\\)l$不能保证减去了全局最大值。
...
因为$T\_c=⌈\\frac{N}{B\_c}⌉=⌈\\frac{4Nd}{M}⌉ \\(，所以FlashAttention总IO为\\)O(N <sup>2d</sup> 2/M)\\(。d一般为64或者128，M通常为100K左右，因此\\)d^2/M$ 远小于1。所以FlashAttention v1的MAC远小于标准的Transformer。
