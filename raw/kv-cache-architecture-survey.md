...
source: "https://zhuanlan.zhihu.com/p/2035408576978629074"
author:
  - "[[银光程序员]]"
published:
created: 2026-05-25
description: "五一本打算更新一年前写的开源引擎 KVCache 实现文档，但梳理下来发现，光是 KVCache 演进的理论部分就已足够撑起一篇独立文章。在 Agent 改变搜索习惯、重塑软件行业的今天，多轮交互带来的长上下文让 KVCache 持…"
tags:
  - "clippings"
---
[收录于 · AIGC](https://www.zhihu.com/column/c_1625207909599617024)
...
![](https://picx.zhimg.com/v2-be2ebefe4510c0eb50c17f7483bef3ff_1440w.jpg)
...
![](https://pic3.zhimg.com/v2-309b7919942eb3ebfcddea180eb2483c_1440w.jpg)

过去几年，大模型 Attention 架构经历了 MHA、MQA、GQA、MLA，再到稀疏注意力、 [线性注意力](https://zhida.zhihu.com/search?content_id=274331021&content_type=Article&match_order=1&q=%E7%BA%BF%E6%80%A7%E6%B3%A8%E6%84%8F%E5%8A%9B&zhida_source=entity) 的演进。从 KVCache 的视角审视，可以归纳为同一个目标：让模型更强的同时，让 KVCache 更小。

### 3.1 压缩 KV 头数 —— MHA → MQA → GQA

![](https://pic3.zhimg.com/v2-36c4c9292366a1440e906696eb4b1884_1440w.jpg)
...
![](https://pic2.zhimg.com/v2-1ad265f7623e925356279b72325b2ef1_1440w.jpg)
...
**3.3.1 [SWA](https://zhida.zhihu.com/search?content_id=274331021&content_type=Article&match_order=1&q=SWA&zhida_source=entity) —— 滑动窗口，截断历史**
...
3.3.2 稀疏选择 —— 选择性保留关键信息

滑动窗口简单高效，但完全丢弃了窗口外的信息。能否不全部丢弃，而是稀疏地选择最重要的 token 保留？

**[NSA](https://zhida.zhihu.com/search?content_id=274331021&content_type=Article&match_order=1&q=NSA&zhida_source=entity)** （Native Sparse Attention, DeepSeek 2025.02）率先验证了这一思路。它将注意力分解为三个并行分支：

- **压缩分支** ：将 token 分块压缩为粗粒度表示，捕获全局上下文
- **精选分支** ：通过 top-k 选择最相关的 token 块，保留关键细节
- **滑动窗口分支** ：保留局部邻近信息

三个分支输出加权融合，以远少于全量的计算覆盖关键上下文。NSA 的设计强调硬件对齐——以连续块为粒度做稀疏选择，匹配 GPU 的块状内存访问模式。 但 NSA 的三分支并行架构也带来了较高的工程复杂度。

**DSA** （DeepSeek Sparse Attention, DeepSeek-V3.2 2025.09）继承 NSA 稀疏核心思路，做了轻量化的工程落地，只保留精选分支的思路：通过轻量级的 lightning indexer，每个 query head 选择最相关的 KV 进行注意力计算。

**3.3.3 CSA + HCA —— 分层混合，集大成者**

![](https://pic4.zhimg.com/v2-5af5580a59dec039247d0ee0009a388d_1440w.jpg)
...
**（1）滑动窗口（所有层都有）—— 存储最近 128 token 的完整 KV**
...
HCA（31 层）将历史序列按 128 token 为一块，通过 token compressor 将整块 **融合为 1 个 compressed KV 向量（** 带 Softmax 的加权融合）：
...
- **压缩方法相同，压缩比不同** ：CSA 是轻量级压缩（4 token → 1），保留更多细节；HCA 是大块压缩（128 token → 1），压缩更激进，丢失更多局部信息。
...
![](https://pic2.zhimg.com/v2-aa9dd17e839f352cc3ba648b402b3ac7_1440w.jpg)

这一思路的演进经历了三个阶段：纯 SSM 架构验证"固定隐状态"的可行性（ [Mamba](https://zhida.zhihu.com/search?content_id=274331021&content_type=Article&match_order=1&q=Mamba&zhida_source=entity) ），线性注意力在保持 O(1) 缓存的同时改善表达力（DeltaNet），最终混合架构在工程落地中找到纯注意力与线性机制的最佳配比（Qwen3.5）。
...
**3.4.2 [Gated DeltaNet](https://zhida.zhihu.com/search?content_id=274331021&content_type=Article&match_order=1&q=Gated+DeltaNet&zhida_source=entity) —— 门控线性注意力**
...
```
固定隐状态: Sₜ = αₜ · Sₜ₋₁ + βₜ · (vₜ - αₜ · Sₜ₋₁ · kₜ) · kₜᵀ
输出:      oₜ = Sₜ · qₜ
...
- `+ βₜ · (vₜ - αₜ·Sₜ₋₁·kₜ) · kₜᵀ` ：Delta Rule 更新：先计算"预测误差" `vₜ - αₜ·Sₜ₋₁·kₜ` （当前真实值减去衰减后记忆矩阵对 kₜ 的预测），再将纠错信号沿 kₜ 方向写入矩阵
- 展开： `- βₜ · (αₜ·Sₜ₋₁·kₜ) · kₜᵀ` （擦除衰减后的旧关联） `+ βₜ · vₜ · kₜᵀ` （写入新关联）
...
| --- | --- | --- |
| 隐状态形态 | 一维向量 h∈ℝᵈ | 二维矩阵 S∈ℝᵈˣᵈ，信息容量更大 |
...
标量量化到 4 bit 已接近极限——再低精度损失会急剧上升。另一类思路是 **不逐标量量化，而是对整个 KV 向量做向量级压缩** 。例如 Google Research 的 [TurboQuant](https://link.zhihu.com/?target=https%3A//arxiv.org/abs/2504.19874) ，通过极坐标变换 + 在线向量量化将 KVCache 压缩到平均每个维度仅需约 3.5 bits（对比 BF16 的 16 bits），实现 5-6 倍压缩，且质量几乎无损。
...
CLA 可以与 GQA/MLA 正交组合，进一步压缩 KVCache。目前主要在学术研究阶段，尚未被主流模型大规模采用。相关工作可参考： [Reducing Transformer Key-Value Cache Size with Cross-Layer Attention](https://link.zhihu.com/?target=https%3A//arxiv.org/abs/2405.12981) （MIT-IBM, 2024，CLA 的原始提出）、 [xKV: Cross-Layer SVD for KV-Cache Compression](https://link.zhihu.com/?target=https%3A//arxiv.org/abs/2503.18893) （2025，基于 SVD 的后训练跨层压缩，兼容 MLA）。
...
![](https://picx.zhimg.com/v2-4053a071f49eadcc9791bc390b6b9483_1440w.jpg)
...
> 注：1M = 1,000,000 tokens。"单 Token KV 大小"= 每层元素数 × 层数 × 2 字节（FP16）。V3 有效 KV 维度为 576（kv\_lora\_rank + qk\_rope\_head\_dim），V4 采用 K=V 共用设计，只缓存一份 512 维向量。V4 另有每层 128 token 滑窗固定开销（约 7.6 MB），长序列下占比可忽略。
...
| --- | --- | --- | --- |
| 头数压缩（3.1） | GQA | 带宽缓解 | 低 |
| 维度压缩（3.2） | MLA | 显存容量大幅释放 | 中（需支持矩阵吸收或Kernel 支持解压） |
| 稀疏注意力（3.3） | CSA/HCA/NSA | 序列级显存释放 | 高（需动态调度和稀疏 Kernel） |
...
- **稀疏注意力** （Sparse Attention）：以 DeepSeek V4 的 CSA/HCA、GLM-5 的 MLA+DSA 为代表，通过分层稀疏机制仅让高相关的 Token 参与计算，在保持全局建模精度的同时，将 KVCache 冗余度压至极限。
...
1. [Qwen3-Next：迈向更极致的训练推理性价比](https://link.zhihu.com/?target=https%3A//qwen.ai/blog%3Fid%3D4074cca80393150c248e508aa62983f9cb7d27cd)
2. [DeepSeek-V4: Towards Highly Efficient Million-Token Context Intelligence](https://link.zhihu.com/?target=https%3A//huggingface.co/deepseek-ai/DeepSeek-V4-Pro/blob/main/DeepSeek_V4.pdf)
3. [MQA/GQA/YOCO/CLA/MLKV笔记: 层内和层间KV Cache共享](https://zhuanlan.zhihu.com/p/697311739)
4. [【DeepSeek 注意力】MHA、MLA、DSA、CSA/HCA——从 V1 到 V4](https://link.zhihu.com/?target=https%3A//mp.weixin.qq.com/s/YqcTnIrGKEYZ2n-sMAelKg)
