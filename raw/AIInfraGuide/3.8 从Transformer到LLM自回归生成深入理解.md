...
用一个简单的数学对比来说明 KV Cache 的价值。假设模型有 $L$ 层，每层的 QKV 投影计算量为 $N \times 3 \times d_{model}^2$（三个矩阵乘法）。无 KV Cache 时，每步重新处理完整前缀，Attention 计算量随前缀长度平方增长；有 KV Cache 时，Decode 阶段只有 1 个 Q token。
...
KV Cache 将 QKV 投影的总计算量从 $O(N^2 d^2)$ 降到了 $O(N d^2)$，节省了 $N$ 倍；同时将自回归生成中的 Attention 总计算量从 $O(N^3 d)$ 降到 $O(N^2 d)$。
