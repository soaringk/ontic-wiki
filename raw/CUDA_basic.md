...
```n
kernel_name<<<4, 8>>>(argument list); // 4 个块，每个块 8 个线程
```
...
```c
kernel_name<<<1,32>>>(argument list); // 1 个 block  kernel_name<<<32,1>>>(argument list); // 32 个 block
```
...
```c
if (con) {     //do something } else {     //do something }
```
...
注意第一个参数是**指针的指针**，一般的用法是首先我们生命一个指针变量，然后调用这个函数：
...
其余套路跟 C 基本一致，cudaMemset 初始化、cudaFree 释放
...
#### <img src="https://intranetproxy.alipay.com/skylark/lark/0/2026/png/180556543/1775819160708-25ebec66-aba8-4aa3-a096-928c59d2452d.png" width="908" title="" crop="0,0,1,1" id="u8d916d8e" class="ne-image" style="color: rgb(85, 85, 85)">
#### 固定内存
...
<img src="https://intranetproxy.alipay.com/skylark/lark/0/2026/png/180556543/1775819338994-d7e14af7-9ff9-4843-ad3e-b5753c906183.png" width="1346" title="" crop="0,0,1,1" id="xOFX5" class="ne-image" style="color: rgb(85, 85, 85); font-size: 16px">
...
#### 零拷贝内存
虽然之前说异构设备之间不能直接访问，对于早期设备，这是肯定的，但是后来，一个例外出现了——零拷贝内存。GPU线程可以直接访问零拷贝内存，这部分内存在主机内存里面，CUDA核函数使用零拷贝内存有以下几种情况：

+ 当设备内存不足的时候可以利用主机内存
+ 避免主机和设备之间的显式内存传输
+ 提高PCIe传输率

前面我们讲，注意线程之间的内存竞争，因为他们可以同时访问同一个内存地址，现在设备和主机可以同时访问一个设备地址了，所以，我们要注意**主机和设备的内存竞争**——当使用零拷贝内存的时候。
...
<img src="https://intranetproxy.alipay.com/skylark/lark/0/2026/png/180556543/1775819665740-0cd0716a-6905-4882-8818-d2b8dc249211.png" width="1588" title="" crop="0,0,1,1" id="uf4d9935f" class="ne-image" style="color: rgb(85, 85, 85); font-size: 16px">

#### 统一内存寻址
Nvidia的同志们还是不停的搞出新花样，CUDA6.0的时候又来了个统一内存寻址，注意不是同一虚拟寻址，提出的目的也是为了简化内存管理。统一内存中创建一个托管内存池（CPU上有，GPU上也有），内存池中已分配的空间可以通过相同的指针直接被CPU和GPU访问，底层系统在统一的内存空间中自动的进行设备和主机间的传输。数据传输对应用是透明的，大大简化了代码。  
...
### 访问对齐
SM执行的基础是线程束，也就是说，当一个SM中正在被执行的某个线程需要访问内存，那么，和它同线程束的其他31个线程也要访问内存，这个基础就表示，即使每个线程只访问一个字节，那么在执行的时候，只要有内存请求，至少是32个字节，所以不使用一级缓存的内存加载，一次粒度是32字节而不是更小。  
...
<img src="https://intranetproxy.alipay.com/skylark/lark/0/2026/png/180556543/1775820738772-da4447b7-2f46-4626-8911-dce915947cf6.png" width="858" title="" crop="0,0,1,1" id="u31db8548" class="ne-image" style="color: rgb(85, 85, 85); font-size: 16px">  
不连续，也不在同一个对齐段上，就得访问两次：

<img src="https://intranetproxy.alipay.com/skylark/lark/0/2026/png/180556543/1775820760844-f1bf9160-c183-4a00-9110-a8cdac7ce6e9.png" width="885" title="" crop="0,0,1,1" id="u2018e0c0" class="ne-image" style="color: rgb(85, 85, 85); font-size: 16px">  
更多情况见：[https://face2ai.com/CUDA-F-4-3-%E5%86%85%E5%AD%98%E8%AE%BF%E9%97%AE%E6%A8%A1%E5%BC%8F/](https://face2ai.com/CUDA-F-4-3-%E5%86%85%E5%AD%98%E8%AE%BF%E9%97%AE%E6%A8%A1%E5%BC%8F/)
